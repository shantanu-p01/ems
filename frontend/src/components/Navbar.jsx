...
const Navbar = () => {
  ...
  const isActive = (path) => {
    // Check if the current route is `/` or `/home` and highlight them as "Home"
    if (location.pathname === "/" || location.pathname === "/home") {
      return path === "/" ? "scale-110 text-yellow-500" : "text-white";
    }
    return location.pathname === path ? "scale-110 text-yellow-500" : "text-white";
  };
